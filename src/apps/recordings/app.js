define(function (require) {
	...
	var app = {
		...
		updateAccount: function (settings) {
			...
			monster.request({
				resource: 'recordings.account.update',
				data: {
					accountId: self.accountId,
					data: settings,
				},
		...
		getEmailWebhook: function (callback) {
			...
			monster.request({
				resource: 'recordings.webhooks.list',
				data: {
					accountId: self.accountId
				},
				success: function (response) {
					...
					monster.request({
						resource: 'recordings.webhooks.get',
						data: {
							accountId: self.accountId,
		...
		reconcileEmailWebhook: function (existing, enabled, email) {
			...
			if (existing) {
				monster.request({
					resource: 'recordings.webhooks.update',
					data: {
						accountId: self.accountId,
				...
				monster.request({
					resource: 'recordings.webhooks.create',
					data: {
						accountId: self.accountId,
						data: data
					},
		...
		deleteEmailWebhook: function (webhookId) {
			...
			monster.request({
				resource: 'recordings.webhooks.delete',
				data: {
					accountId: self.accountId,
					webhookId: webhookId
				},
		...
		recordingEndpoints: {
			user: {
				...
				updateRequest: 'recordings.user.update',
				idKey: 'userId',
			...
			device: {
				...
				updateRequest: 'recordings.device.update',
				idKey: 'deviceId',
				getFields: function (doc) {
					return {
						name: doc.name,
			...
			function fetchPage(startKey) {
				...
				var request = {
					resource: userId && userId !== 'all'
						? 'recordings.recordings.listByUser' : 'recordings.recordings.list',
					data: {
						accountId: self.accountId,
		...
		bulkDeleteRecordings: function (recordingIds, callback) {
			...
			_.each(recordingIds, function (recordingId) {
				requests[recordingId] = function (cb) {
					monster.request({
						resource: 'recordings.recordings.delete',
						data: {
							accountId: self.accountId,
